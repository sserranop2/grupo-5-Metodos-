...
def _alcanzado_umbral_en_trayectoria(vals_pu, umbral=80.0):
    return np.any(vals_pu >= umbral)

def _frecuentista_intervalo(p_hat, N, z=1.96):
    se = math.sqrt(max(p_hat * (1.0 - p_hat) / max(N, 1), 0.0))
    lo = max(0.0, p_hat - z * se)
    hi = min(1.0, p_hat + z * se)
    return p_hat, se, lo, hi

def _bayes_uniforme_intervalo(k, N, alpha0=1.0, beta0=1.0, qlo=0.025, qhi=0.975):
    from scipy.stats import beta as beta_dist
    a = alpha0 + k
    b = beta0 + (N - k)
    mean = a / (a + b)
    lo = beta_dist.ppf(qlo, a, b)
    hi = beta_dist.ppf(qhi, a, b)
    return mean, lo, hi

def _formatea_linea(metodo, N, k, p_freq, se, lo_f, hi_f, mean_b, lo_b, hi_b):
    pct = 100.0 * p_freq
    se_pct = 100.0 * se
    lo_f_pct, hi_f_pct = 100.0 * lo_f, 100.0 * hi_f
    mean_b_pct, lo_b_pct, hi_b_pct = 100.0 * mean_b, 100.0 * lo_b, 100.0 * hi_b
    return (
        f"[{metodo}] N={N}, k={k}\n"
        f"  Frecuentista: p̂ = {pct:.2f}% ± {se_pct:.2f}%  "
        f"(95% CI: {lo_f_pct:.2f}%, {hi_f_pct:.2f}%)\n"
        f"  Bayes (Beta(1,1)): media = {mean_b_pct:.2f}%  "
        f"(95% CrI: {lo_b_pct:.2f}%, {hi_b_pct:.2f}%)\n"
    )

def simulacion_2d_probabilidad(
    N=1000,
    umbral_pu=80.0,
    tmax=30.0,
    dt_sde=1e-3,
    ...
    resultados = []
    np.random.seed(seed_sde)
    bloque = max(1, min(250, N))
    k_sde = 0
    hechos = 0
    while hechos < N:
        n_blk = min(bloque, N - hechos)
        t_sde, Y_blk = sde_rk2_trayectorias(
            y0=[U_inicial, Np_inicial, Pu_inicial],
            t0=0.0,
            t_fin=tmax,
            dt=dt_sde,
            n_tray=n_blk
        ...
        for m in range(n_blk):
            if _alcanzado_umbral_en_trayectoria(Y_blk[m, :, 2], umbral=umbral_pu):
                k_sde += 1
        hechos += n_blk

    p_sde = k_sde / N
    p_sde, se_sde, lof_sde, hif_sde = _frecuentista_intervalo(p_sde, N)
    meanb_sde, lob_sde, hib_sde = _bayes_uniforme_intervalo(k_sde, N)
    resultados.append(_formatea_linea("SDE (2.b)", N, k_sde, p_sde, se_sde, lof_sde, hif_sde, meanb_sde, lob_sde, hib_sde))

    rng = np.random.default_rng(seed_gill)
    k_g = 0
    for _ in range(N):
        ts, Ys = gillespie_un_path(tmax, [U_inicial, Np_inicial, Pu_inicial], rng)
        if _alcanzado_umbral_en_trayectoria(Ys[:, 2], umbral=umbral_pu):
            k_g += 1

    p_g = k_g / N
    p_g, se_g, lof_g, hif_g = _frecuentista_intervalo(p_g, N)
    meanb_g, lob_g, hib_g = _bayes_uniforme_intervalo(k_g, N)
    resultados.append(_formatea_linea("Gillespie (2.c)", N, k_g, p_g, se_g, lof_g, hif_g, meanb_g, lob_g, hib_g))

    comentario = (
        f"Definición de evento: alcanzar Pu >= {umbral_pu:.1f} en cualquier instante dentro de 0–{tmax} días.\n"
        "Intervalo frecuentista: aproximación normal (±1.96·SE). "
        "Bayes: posterior Beta(1+k, 1+N−k) con prior uniforme.\n"
    )

    with open("2.d.txt", "w", encoding="utf-8") as f:
        f.write("2.d — Probabilidad de concentración crítica (resultado en porcentajes)\n")
        f.write(comentario + "\n")
        for linea in resultados:
            f.write(linea + "\n")

    print(f"==== 2.d — Probabilidad de concentración crítica (Pu >= {umbral_pu:.1f}) ====")
    print(comentario)
    for linea in resultados:
        print(linea, end="")
    print("\nArchivo '2.d.txt' generado.")

simulacion_2d_probabilidad(N=1000, umbral_pu=80.0, tmax=30.0, dt_sde=1e-3)
